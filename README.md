Перевод терминов.
* reclaim - Можно перевести и как освобождение, и как восстановление. Был выбран первый вариант, так как логика термина в данном случае предполагает такую интерпретацию
* eviction - вытеснение
* swap - подкачка
* thrashing - пробуксовка
* cgroup - оставил без перевода, так как практически имя собственное
* dirty/clean - грязные/чистые, применительно к измененным/неизмененным страницам памяти.
* memory starvation - нехватка памяти
* demand paging - пока не переводил, потому что неясно как. Предлагаемые гуглом "пейджинг по запросу" или "подлистывание по запросу" применять не хочется
...
Например, есть страницы («блоки» памяти, обычно 4 КБ), в которых находится код для каждого процесса, запущенного на вашем компьютере. Есть также страницы, отвечающие за кэширование данных и метаданных, связанных с файлами, к которым обращаются эти программы, для ускорения будущего доступа. Они являются частью страничного кэша, назовем их **файловой памятью**.

Память, аллоцированная из этого (выполняемого) кода, например, выделенная с помощью `malloc()`, или при использовании `mmap()` c флагом `MAP_ANONYMOUS` - это **анонимная память**. Называется она так, потому что не подкреплена файлами, располагаемыми на диске.

Иначе говоря, процесс отображает память в Linux'e с помощью системного вызова `mmap(2)`. Данная память может быть как подкреплена реальным файлом на диске, так и нет. Во втором случае получится пустой раздел памяти, называемый анонимным.
...
Одной из концепций Linux является утверждение: неиспользуемая память - бесполезная память. Соответственно, практически вся память, к которой происходило обращение, остается в виде кэша в оперативной памяти. Это положительно сказывается на производительности, так как уменьшает дисковые операции, однако, рано или поздно системе требуется принимать решение, какую из размещенных в памяти страниц нужно освобождать.

И один из самых фундаментальных вопросов при рассмотрении конкретного типа памяти — можно ли ее освободить или нет. «Освобождение» (reclaim) здесь означает, что система может, не теряя данных, очистить страницы этого типа из физической памяти.

Для некоторых типов страниц это обычно довольно тривиально. Например, в случае чистой (clean) (немодифицированной) файловой страницы - мы можем удалить страницу, так как изменений не было, и нам не требуется что-то сохранять на диск.

Для некоторых типов страниц это возможно, но уже чуть сложнее. Например, в случае грязной (dirty) (модифицированной) файловой страницы - мы не можем просто удалить страницу, потому что на диске еще нет текущих изменений. Таким образом, нам нужно либо отклонить освобождение, либо сначала сбросить изменения на диск, а уже после этого освободить данный участок памяти.

Для некоторых типов страниц это в принципе невозможно. Например, в случае с анонимными страницами, они существуют только в памяти и не имеют резервного хранилища. Фактически, они с момента создания являются грязными, но у них нет места на диске, куда бы их можно было сохранить.
...
* **есть swap**: мы можем сбросить в swap редко используемую анонимную память, которая может использоваться только в течение небольшой части жизненного цикла процесса, что позволит нам использовать эту память, например, для повышения частоты попаданий в кэш.

* **нет swap'a**: у нас нет такой возможности, так как страница заблокирована в памяти. Хотя это может не сразу проявиться как проблема, при некоторых нагрузках это может представлять собой серьезное падение производительности из-за "старых" анонимных страниц, отнимающих место, которое можно использовать для более важных вещей.

**При умеренной/высокой конкуренции за память**

* **есть swap*: все типы памяти имеют одинаковую вероятность быть сброшенными в swap. Это означает, что у нас больше шансов успешно освободить страницу, то есть мы можем освободить страницы, которые не будут промахом по кэшу (page fault) в ближайшее время (не возникнет эффект **пробуксовки**).
...
>
>![thrashing](./pics/thrashing.png)
>
> На графике показана зависимость утилизации процессора от количества процессов в системе. Она растет до определенного момента, после которого большую часть времени система занята вытеснением страниц в swap и загрузкой из него ранее вытесненного. 

* **нет swap'a**: анонимные страницы заблокированы в памяти, так как их некуда сбрасывать. Вероятность успешного долгосрочного освобождения страниц ниже, так как у нас есть только некоторые типы памяти, которые могут быть освобождены в принципе. Риск пробуксовки выше. Может показаться, что это все равно будет лучше, так как это поможет избежать необходимости обращения к диску, но это не так — мы просто переносим дисковый ввод-вывод подкачки на удаление горячих кэшей (hot page caches) и сегментов кода, которые нам скоро понадобятся.

**При временных всплесках использования памяти**

* **есть swap**: Система более устойчива к временным всплескам, но, в случаяе серьезной нехватки памяти, период от начала пробуксовки до OOM killer'a может быть увеличен. Лучше видно причины возникновения данных всплесков и можно выполнять контролируемое вмешательство.

* **нет swap'a**: OOM killer срабатывает быстрее, так как анонимные страницы заблокированы в памяти и не могут быть освобождены. Большая вероятность пробуксовки, но время между переполнением и OOM'ингом сокращается. В зависимости от приложения это может быть лучше или хуже. Например, для приложение на основе очередей лучше быстрый переход от пробуксовки к OOM killer'у. Тем не менее, так как OOM killer включается только в момент серьезной нехватки памяти, полагаться на такой алгоритм не слишком хорошая идея.
...
В большинстве случаев, нескольких гигабайт swap'a достаточно для нормального функционирования системы. Однако, можно создать несколько тестовых машин, и запустить предполагаемую, желательно неровную, с пиками, нагрузку. Если система не находится в состоянии нехватки памяти (в этом случае тест не будет показательным), можно определить сколько MB swap'a в среднем задействовано. Это будет минимальным значением + некий буфер для возможного изменения нагрузки. 
...
![page hierarchy](./pics/X86_Paging_64bit_red.svg)
...
<details>
<summary>Политика записи при кэшировании</summary>
...
Для того, чтобы определить, какую страница требуется вытеснить, используется LRU (least recently used) алгоритм. В настоящее время была добавлена  усложненная версия **multi-generational LRU**
...
Допустим, пользовательски процесс прочитал некоторые данные с диска. Это действие запустило загрузку данных в кэш. Так как это был первый раз, когда ядро обращалось к этой странице ***h***, она помещается в начало inactive-очереди cgroup'ы процесса.
...
**zram** (раньше назывался compcache, compressed cache) - модуль ядра Linux, используемый для создания блочного устройства в оперативной памяти, RAM-диск с комрессией на лету. Обычно используется для подкачки (swap) или как RAM-диск общего назначения, в основном используемый для хранения временных файлов.

Первоначально созданное блочноe устройство не резервирует память. Только когда требуется выгрузить страница в swap, она сжимается и помещается в zram. Данное блочное устройство динамически изменяет размер выделенной памяти по необходимости.
...
#### Для самых упорных. Кого вытесняем
...
* https://systemsresearch.io/posts/a9455fa9e/
