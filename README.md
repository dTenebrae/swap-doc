...
Одним из свойств отображения памяти является возможность работы с файлами, чей размер превышает физический размер RAM.

На изображении ниже показано, как выглядит виртуальная память процесса. У каждого процесса есть область, куда **mmap()** отображает файлы.

![process vm](./pics/mmap.png)
